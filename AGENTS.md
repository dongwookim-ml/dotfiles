...
The exception is `Codex/.Codex/skills/<name>`, linked as a whole directory so
files added inside a skill appear without relinking. `~/.Codex/skills/` itself
...
| **Codex** | Shared settings.json, global AGENTS.md, hooks, and custom skills |
...
`~/.Codex/settings.local.json`. Never commit tokens, keys, or webhook URLs.

For Codex, keep `model`, `statusLine`, and `mcpServers` in
